...
Orders Router
Core business logic:
1. Vendor places order (POST /orders/)
2. System checks inventory for all line items
3. Reserves stock
4. Calculates Haversine distance from warehouse → delivery coords
5. Calls ETA service with the computed distance
6. Creates a Shipment record with full ETA breakdown
7. Returns the confirmed order + tracking number to the vendor
"""
...
async def place_order(payload: OrderCreate, db: Session = Depends(get_db)):
    ...
    for line in payload.items:
        inv = db.query(InventoryItem).filter(InventoryItem.sku == line.sku, InventoryItem.vendor_id == payload.vendor_id).first()
        if not inv:
            raise HTTPException(status_code=404, detail=f"SKU '{line.sku}' not found in inventory for vendor '{payload.vendor_id}'.")
        ...
        # Use the first item's warehouse as origin (assumes single-warehouse per order)
    ...
    # ── Step 3: Reserve inventory and create order items ──────────────────────
